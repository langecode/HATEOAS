...
public class DKPersonSample implements NonSensitiveSemanticID {
    ...
    public String adjustHumanReadableNonSensitiveID(long sequence) {
        if (sid.isPresent()) {
            sid = Optional.of(sid.get() + NonSensitiveSemanticID.SEPARATOR + Long.toString(sequence));
        } else {
            sid = Optional.of(constructSID() + NonSensitiveSemanticID.SEPARATOR + Long.toString(sequence));
        }
        return sid.get();
    }

    private String constructSID() {
        char separator = NonSensitiveSemanticID.SEPARATOR;
    ...
    private String getSIDPart(String part, char separator) {
        int length = NonSensitiveSemanticID.LENGTH;
